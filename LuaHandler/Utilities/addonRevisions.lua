...
function AddonRevs.LoadAddonInfoRev2(filepath, _VFSMODE)
	...
	Spring.Echo("LOADING AS REV2", filepath)
	tcopy(ki, rvalue)
	return false, ki
end
...
function AddonRevs.NewAddonRev2()
	...
	--// addon related methods
		tmergein(addonEnv.addon, {
			...
			DisableWidget = function(_, name) handler:Disable(name) end,
			EnableWidget = function(_, name) handler:Enable(name) end,
...
function AddonRevs.NewAddonRev1()
	...
	addonEnv._handler = handler
	addonEnv["_" .. handler.name] = handler
	...
	h.DisableWidget = function(_, name) handler:Disable(name) end
	h.EnableWidget = function(_, name) handler:Enable(name) end
