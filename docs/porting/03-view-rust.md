---
name: Phase 3 — View logic → Rust
description: Port the editor UI to the native Rust plugin, driving the engine's RmlUi
---
...
Known defects: [03-view-rust-issues.md](03-view-rust-issues.md).
The user's live test ledger: [user-testing-issues.md](user-testing-issues.md).
...
- **`mod.rs` holds no logic.** A `mod.rs` is *only* `mod` and `pub use` lines —
  no functions, no `struct`/`enum`/`impl` definitions, no real code. A module's
  code lives in siblings it re-exports (e.g. `actions/{action,run,project,…}.rs`,
  with `actions/mod.rs` a thin re-export). Fat `mod.rs` files must be decoupled.
...
| unit / feature picker   | done — grid + RTT thumbnails (silhouette; texturing TODO) |
| dialogs                 | asset picker, colour picker, file dialog, new-project dialog |
...
| Objects | Units / Features / Properties | done (grids have RTT thumbnails, silhouette-only) |
| Objects | Collision | done — follows selection; scale sync (Sphere/Cylinder); field visibility |
| Map | Settings / Terrain / Texture / Metal / Grass | done (Texture: paint only, no material picker) |
...
- [ ] Objects → Units / Features: grid + 3D RTT thumbnails. Lua renders these
      ...
      -> I thought you already did something here? Please check. The white silouhete is still a big issue though.
...
- [ ] Objects → Properties, Collision (project model; now unblocked).
      Properties is done; Collision is done.
- [ ] Selecting features isn't updating Objects->Properties/Collisions (nothing appears)
- [~] When in add/brush object mode, render the to-be-placed object. Done as a
      cursor **ghost ring** (`states/highlight.rs::draw_cursor_ring`, called from
      each state's `draw_world`): the drop point + facing in Set mode, the scatter
      reach in Brush mode, and a brush-reach ring for the map brushes. The full 3D
      model preview still needs the engine model-draw phase (the immediate-mode
      ring renders in `draw_world`; the engine model drawer does not).
...
- [x] Toolbar action buttons (new/open/save/export/import + copy/cut/paste), plus
      their Ctrl-key hotkeys. Ported as `native/src/sbc/actions/` (thin `mod.rs`;
      `action.rs` metadata, `run.rs` execute logic, `project.rs` new-project,
      `clipboard.rs`). The bar builds in `view.rs::build_action_bar`; dialogs are
      `panels/file_dialog.rs` (Load/SaveAs/Import/Export) and
      `panels/new_project_dialog.rs`. The command layer is **engine-verified**:
      `objects/tests/test_clipboard.rs` (`clipboard_copy_paste`, `delete_selection`)
      runs the actions against the live models in the ASAN integration boot.
      A systemic bug this caught: every command was built with `envelope()` (fields
      under `opts`) but the project/object commands read fields off `data` — they
      all needed `envelope_fields()`, so Save/Load/Import/Export/Delete/Cut would
      have silently no-op'd. Fixed. (Dialog *UI* still needs a live-render pass.)
- [x] Native chonsole: removed its dead `context_render` call (the engine renders
      every RmlUi context itself; a plugin's call submits geometry outside the
      frame, where it is dropped).
