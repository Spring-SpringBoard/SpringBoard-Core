return {
	...
	fragment = [[
		...
		void main() {
			...
			//New slope shader
			vec4 norm = texture2D(tex2, texCoord);
			...
			//gl_FragColor = mix(vec4(1.0), gl_FragColor, getTexel(tex1, texCoord).r * 0.35 + 0.7); //Mixing with LoS info texture to brighten up the area inside radar vision.
			//gl_FragColor.a = 0.3;
