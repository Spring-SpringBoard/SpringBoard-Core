...
def gallery(run_state: "RunState") -> None:
    ...
    run_state.click(left + GALLERY["dev_tab_x"], TAB_Y, delay=Delay.SETTLE)
    run_state.click(*editor_point(left, "dev", "gallery"), delay=Delay.READY)
    ...
    run_state.click(*panel_point(left, GALLERY["choice"]), delay=Delay.FRAME)
    run_state.golden("choice-open", park=False, tolerance=CURSOR_IN_SHOT)
    run_state.click(*panel_point(left, dropdown_option(GALLERY["choice"], 1)), delay=Delay.DIALOG)
    ...
    run_state.click(*panel_point(left, GALLERY["empty"]), delay=Delay.FRAME)
    ...
    run_state.click(*panel_point(left, GALLERY["bool_off"]), delay=Delay.SETTLE)
    ...
    for point, value in (
        ...
        run_state.fill_text(*panel_point(left, point), value, click_delay=Delay.FRAME, commit_delay=Delay.SETTLE)
    ...
    run_state.click(*panel_point(left, GALLERY["string"]), delay=Delay.FRAME)
...
def gallery_pickers(run_state: "RunState") -> None:
    ...
    run_state.click(*panel_point(left, GALLERY["color"]), delay=Delay.READY)
    ...
    run_state.move(color_square[0] + 8, color_square[1] + 8, delay=Delay.SETTLE)
    run_state.release(color_square[0] + 8, color_square[1] + 8)
    run_state.golden("colour-picked", crop=None)
    run_state.click(*dialog_point(run_state, DIALOG["color_ok"]), delay=Delay.DIALOG)
    ...
    run_state.click(*panel_point(left, GALLERY["asset"]), delay=Delay.LOAD)
    packs = run_state.golden("asset-packs", crop=None)

    run_state.click(*dialog_point(run_state, DIALOG["asset_first_cell_gallery"]), delay=Delay.READY)
    ...
    run_state.click(*dialog_point(run_state, DIALOG["asset_up"]), delay=Delay.READY)
    ...
    run_state.click(*dialog_point(run_state, DIALOG["asset_first_cell_gallery"]), delay=Delay.READY)
    run_state.click(*dialog_point(run_state, DIALOG["asset_first_cell_gallery"]), delay=Delay.DIALOG)
    run_state.golden("asset-selected", crop=None)
    run_state.click(*dialog_point(run_state, DIALOG["asset_ok_gallery"]), delay=Delay.READY)
...
def gallery_tooltips(run_state: "RunState") -> None:
    ...
    run_state.move(*panel_point(left, GALLERY["tooltip_parking"]), delay=Delay.DIALOG)
    ...
    run_state.move(*panel_point(left, GALLERY["number"]), delay=Delay.READY)
    hovered = run_state.golden("numeric-tooltip", park=False)
    box = (20, 200, 460, 400)
    run_state.assert_region_pixels(empty, hovered, box, min_changed=800)

    # And it goes away again.
    run_state.move(*panel_point(left, GALLERY["tooltip_parking"]), delay=Delay.READY)
...
def gallery_dialogs(run_state: "RunState") -> None:
    ...
    run_state.click(*panel_point(left, TOOLBAR["new_project"]), delay=Delay.POLL)
    original = run_state.golden("new-project")
    run_state.click(*dialog_point(run_state, DIALOG["new_project_name"]), delay=Delay.POLL)
    run_state.type_text("E2E project")
    run_state.key("Return", delay=Delay.POLL)
    for field, value in (("new_project_size_x", "12"), ("new_project_size_y", "14")):
        run_state.fill_text(
            *dialog_point(run_state, DIALOG[field]),
            value,
            click_delay=Delay.POLL,
            commit_delay=Delay.POLL,
        )
    ...
    run_state.click(*dialog_point(run_state, DIALOG["file_up"]), delay=Delay.POLL)
...
def _open_gallery(run_state: "RunState") -> int:
    run_state.focus()
    left = panel_left(run_state)
    run_state.click(left + GALLERY["dev_tab_x"], TAB_Y, delay=Delay.POLL)
    run_state.click(*editor_point(left, "dev", "gallery"), delay=Delay.POLL)
    return left
