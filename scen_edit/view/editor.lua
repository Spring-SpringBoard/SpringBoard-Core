...
function Editor:_CreateRmlUiDialog(opts)
    ...
    -- Set dialog size and position via style attribute
    local dialogBody = self.document:GetElementById("dialog-window")
    if dialogBody then
        local width = opts.width or 550
        local height = opts.height or 500
        local x = opts.x or "25%"
        local y = opts.y or "20%"

        local xStr = type(x) == "string" and x or (tostring(x) .. "px")
        local yStr = type(y) == "string" and y or (tostring(y) .. "px")

        local style = string.format("width: %dpx; height: %dpx; left: %s; top: %s;",
            width, height, xStr, yStr)
