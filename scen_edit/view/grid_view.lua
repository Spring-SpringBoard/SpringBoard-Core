...
function GridView:_OnValidateSelectItem(obj, itemIdx, selected)
    ...
    if self.layoutPanel then
        item = self.layoutPanel.children[itemIdx]
    else
        -- RmlUi mode
        item = self.childItems[itemIdx]
    end
    return item
end
...
function GridView:NewItem(tbl)
    ...
    if self.layoutPanel then
        ...
        -- Add to childItems array for RmlUi rendering
        -- Don't call _UpdateRmlUiGrid here if we're batching
        if not self._batchingUpdates then
            table.insert(self.childItems, item)
        else
            -- Batching - just add to array, update will happen in EndMultiModify
            table.insert(self.childItems, item)
        end
    end
...
function GridView:_UpdateRmlUiGrid()
    ...
    -- Find the correct document - use editor's document if available, otherwise use main document
    local document = nil
    if self.editor and self.editor.document then
        document = self.editor.document
    elseif SB.view and SB.view.mainDocument then
        document = SB.view.mainDocument
    end
    ...
    local gridContainer = document:GetElementById(self.gridId)
    if not gridContainer then
        -- Grid container not in DOM yet or editor not visible - this is normal
        return
    end

    -- Generate HTML for all child items
    local html = ""
    if #self.childItems > 0 then
        -- Log all items to see their paths
        for i, item in ipairs(self.childItems) do
        end
    end
    for idx, item in ipairs(self.childItems) do
        ...
        elseif item.image then
            -- Wrap image in container for aspect ratio control
            html = html .. '<div class="grid-item-image-container">'
            ...
            if type(imagePath) == "string" then
                ...
                if imagePath:sub(1, 1) == "!" then
                    ...
                    html = html .. string.format('<texture src="%s" class="grid-item-image" alt="%s"/>', imagePath, caption)
                else
                    -- Regular file path - convert to relative path from RML document
                    -- RML documents are in scen_edit/view/rml/, so we need ../../../ to get to root
                    if not imagePath:match("^%.%.") and not imagePath:match("^http") then
                        imagePath = "../../../" .. imagePath
                    end
                    html = html .. string.format('<img src="%s" class="grid-item-image" alt="%s"/>', imagePath, caption)
                end
            elseif type(imagePath) == "number" then
                ...
                html = html .. string.format('<texture src="!%d" class="grid-item-image" alt="%s"/>', imagePath, caption)
            else
                ...
                html = html .. string.format('<texture src="!%s" class="grid-item-image" alt="%s"/>', tostring(texId), caption)
            end
        ...
        if item.caption then
            -- Truncate caption to first 10 characters for display
            local displayCaption = item.caption
            if #displayCaption > 10 then
                displayCaption = displayCaption:sub(1, 10) .. "..."
            end
            html = html .. string.format('<div class="grid-item-label" title="%s">%s</div>', item.caption, displayCaption)
        end
    ...
    for idx, item in ipairs(self.childItems) do
        ...
        if item.__add_brush and item.OnAddClick then
            local addBtn = document:GetElementById(itemId .. "-add-btn")
            if addBtn then
        ...
        if item.OnRemoveClick then
            local removeBtn = document:GetElementById(itemId .. "-remove-btn")
            if removeBtn then
        ...
        if not item.__add_brush then
            local itemElement = document:GetElementById(itemId)
            if itemElement then
