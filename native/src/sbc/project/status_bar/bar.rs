...
//!
//! The higher level owns a single `#project-status-root` div in the panel
//! document; everything else — markup, wiring, and the effects — lives here. The
//! panel manager only renders the caption each frame and forwards any queued
//! actions, so this feature stays self-contained.
...
/// One button in the bar: its element id, caption, and the action it queues.
const BUTTONS: [(&str, &str, StatusBarAction); 4] = [
    (OPEN_ID, "Open Project", StatusBarAction::OpenProject),
