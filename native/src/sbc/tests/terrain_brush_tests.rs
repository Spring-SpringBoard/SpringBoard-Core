...
fn terrain_level_brush(ctx: &mut TestCtx) -> Result<(), String> {
    ...
    let baseline = 10.0_f32;
    {
        ...
        let span = (sx as f32) * SQUARE_SIZE;
        let mut x = cx - span;
        while x <= cx + span {
            let mut z = cz - span;
            while z <= cz + span {
                let _ = terrain.set_height_map(x, z, baseline, 1.0);
                z += SQUARE_SIZE;
            }
            x += SQUARE_SIZE;
        }
...
/// only recomputes it inside `RecalcArea`, which a raw `add_height_map` loop does
/// not trigger — so this is the *recalc* signal, distinct from `get_ground_height`
/// (which reflects the corner-height write regardless of recalc).
