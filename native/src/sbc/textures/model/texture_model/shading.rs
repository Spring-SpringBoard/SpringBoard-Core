...
const SHADING_DEFS: &[ShadingDef] = &[
    ShadingDef { name: "specular", engine_name: "$ssmf_specular", bind_type: "$ssmf_specular", slot: 0, needs_mipmap: false },
    ShadingDef { name: "emission", engine_name: "$ssmf_emission", bind_type: "$ssmf_emission", slot: 0, needs_mipmap: false },
    ShadingDef { name: "refl", engine_name: "$ssmf_sky_refl", bind_type: "$ssmf_sky_refl", slot: 0, needs_mipmap: false },
    ShadingDef { name: "splat_distr", engine_name: "$ssmf_splat_distr", bind_type: "$ssmf_splat_distr", slot: 0, needs_mipmap: false },
    ShadingDef { name: "splat_normals0", engine_name: "$ssmf_splat_normals:0", bind_type: "$ssmf_splat_normals", slot: 0, needs_mipmap: true },
    ShadingDef { name: "splat_normals1", engine_name: "$ssmf_splat_normals:1", bind_type: "$ssmf_splat_normals", slot: 1, needs_mipmap: true },
    ShadingDef { name: "splat_normals2", engine_name: "$ssmf_splat_normals:2", bind_type: "$ssmf_splat_normals", slot: 2, needs_mipmap: true },
    ShadingDef { name: "splat_normals3", engine_name: "$ssmf_splat_normals:3", bind_type: "$ssmf_splat_normals", slot: 3, needs_mipmap: true },
    ShadingDef { name: "detail", engine_name: "$detail", bind_type: "$detail", slot: 0, needs_mipmap: true },
];
...
impl ShadingStore {
    ...
    pub(crate) fn surface(&self, name: &str) -> Option<&Surface> {
        self.shading.iter().find(|(n, _)| n == name).map(|(_, slot)| &slot.surface)
    }

    pub(crate) fn texture(&self, name: &str) -> Option<ShadingTexture> {
        self.shading.iter().find(|(n, _)| n == name).map(|(_, slot)| ShadingTexture {
    ...
    pub(crate) fn dirty(&self, name: &str) -> bool {
        self.surface(name).map(|s| s.borrow().dirty).unwrap_or(false)
    }
    ...
    fn assign(&mut self, def: &'static ShadingDef, source: &Texture) {
        ...
        let (width, height) = match gfx.texture_info(source) {
            ...
            other => {
                log::debug!("shading {}: skipped source {source}, texture_info {other:?}", def.name);
                return;
            }
        };
